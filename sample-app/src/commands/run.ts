...
export default class Run extends BaseCommand {
	...
	static examples = [
		'$ lisk-sample-app run',
		'$ lisk-sample-app run -l debug -d ./data',
	];
	...
	public async run(): Promise<void> {
		...
		config.app.label = label;
		if (inputFlags['data-dir']) {
			config.app.dataDirectory = inputFlags['data-dir'];
			config.components.logger.logFileName = `${inputFlags['data-dir']}/${label}/logs/lisk.log`;
		}
		// tslint:disable-next-line no-let
		let app;
		try {
			app = createApp(genesisBlock, config);
		} catch (errors) {
			this.error(errors);
		}
