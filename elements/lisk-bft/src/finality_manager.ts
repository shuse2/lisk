...
export class FinalityManager extends EventEmitter {
	...
	public constructor({
		...
		this.processingThreshold = this.activeValidators * BFT_ROUND_THRESHOLD - 1;

		// Maximum headers to store (5 rounds)
		this.maxHeaders = this.activeValidators * 5;
	...
	public async updatePreVotesPreCommits(
		...
		if (!validator) {
			throw new Error(`Generator ${generatorPublicKey.toString('base64')} is not in validators set`);
		}

		// If validator cannot vote, it cannot vote on the block
		if (!validator.canVote) {
			return false;
		}

		const minimumBFTHeight = stateStore.chain.lastBlockHeaders[stateStore.chain.lastBlockHeaders.length - 1].height;
	...
	private _decodeVotingLedger(bftVotingLedgerBuffer: Buffer | undefined): VotingLedgerMap {
		...
		const validators = votingLedger.validators.reduce((prev: dataStructures.BufferMap<ValidatorState>, curr) => {
